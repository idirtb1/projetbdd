...
<div class="content-wrap">
    ...
    <section id="about-us" class="clearfix">
		...
		if (isset($_GET["artiste"])) {
			...
			die("Vous devez indiquer un artiste pour consulter ses concerts");
		}
		?>
            </p>

            <div class="row no-bottom-margin">
				<h1> Voici les concerts de votre artiste : </h1>
				...
				$req='SELECT* FROM concerts, artistes WHERE concerts.IdArtiste='.$nom_artiste.' GROUP BY IdConcert ;';
				$res=mysqli_query($connexion, $req);
				while ($enreg_artiste=mysqli_fetch_array($res)){
					$lieu_concert=$enreg_artiste['LieuConcert'];
					$date_concert=$enreg_artiste['DateConcert'];
					echo '<tr>';
					echo '<td>';
					echo $lieu_concert;
					echo '</td>';
